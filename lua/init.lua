...
function Mark:register_mark(mark, line, col, bufnr)
	...
	--
	if not Mark.is_lower(mark) or mark:byte() > buffer.lowest_available_mark:byte() then
		return
	end
...
function Mark:refresh(force)
	...
	for mark, _ in pairs(Mark.buffers[bufnr].placed_marks) do
		if api.nvim_buf_get_mark(bufnr, mark)[1] == 0 then
	...
	for _, data in ipairs(vim.fn.getmarklist()) do
		...
		if Mark.is_upper(mark) and pos[1] == bufnr and (force or not cached_mark or pos[2] ~= cached_mark.line) then
	...
	for _, data in ipairs(vim.fn.getmarklist("%")) do
		...
		if Mark.is_lower(mark) and (force or not cached_mark or pos[2] ~= cached_mark.line) then
			Mark:register_mark(mark, pos[2], pos[3], bufnr)
		end
	end
